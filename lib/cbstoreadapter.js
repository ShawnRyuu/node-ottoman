...
function _buildFilterExprs(filters, expressions, root) {
  var SPECIAL_KEYS = ['$exists', '$missing', '$contains', '$like', '$in', '$not', '$rightOpenRange', '$allFrom', '$anyFrom'];
  var BOOLEAN = ['or', 'and'];
  if (!root) {
    root = '';
  }
  ...
  for (var i in filters) {
    if (Object.prototype.hasOwnProperty.call(filters, i)) {
      ...
      if (filters[i].$like) {
        expressions.push('LOWER(' + ident + ') LIKE ' + 'LOWER(\'' + filters[i].$like + '\')');
      }
      if (filters[i].$not) {
        expressions.push(ident + ' != \'' + filters[i].$not + '\'');
      }
      if(filters[i].$in) {
        var keys = Object.keys(filters[i].$in);
        var arr = [];
        for(var j = 0; j < keys.length; j++) {
          arr.push(filters[i].$in[j]);
        }
        expressions.push('(' + ident + ' IN [\'' + arr.join("','") + '\'])');
      }
      ...
      if (filters[i].$allFrom) {
        var keys = Object.keys(filters[i].$allFrom);
        var arr = [];
        for(var j = 0; j < keys.length; j++) {
          arr.push(filters[i].$allFrom[j]);
        }
        if (typeof filters[i].$allFrom[0] === 'string') {
          expressions.push('EVERY x IN [\''+ arr.join("','") + '\'] SATISFIES x IN ' + ident +' END');
        } else if (typeof filters[i].$allFrom[0] === 'number') {
          expressions.push('EVERY x IN ['+ arr.join(',') + '] SATISFIES x IN ' + ident + ' END');
        } else {
          // throw exception.
        }
      }
      if (filters[i].$anyFrom) {
        var keys = Object.keys(filters[i].$anyFrom);
        var arr = [];
        for(var j = 0; j < keys.length; j++) {
          arr.push(filters[i].$anyFrom[j]);
        }
        if (typeof filters[i].$anyFrom[0] === 'string') {
          expressions.push('ANY x IN ' + ident + ' SATISFIES x IN [\''+ arr.join("','") + '\'] END');
        } else if (typeof filters[i].$anyFrom[0] === 'number') {
          subs.push('ANY x IN ' + ident + ' SATISFIES x IN ['+ arr.join(',') + '] END');
        } else {
          // throw exception.
        }
      }
      if (filters[i].$rightOpenRange) {
        if (typeof filters[i].$rightOpenRange[0] === typeof filters[i].$rightOpenRange[1]) {
          if (typeof filters[i].$rightOpenRange[0] === 'string') {
            expressions.push(
              ident + '>=\'' + filters[i].$rightOpenRange[0].replace('\'', '\\\'')
              + '\' AND ' + ident + '<\''
              + filters[i].$rightOpenRange[1].replace('\'', '\\\'') + '\''
            );
          } else if (typeof filters[i].$rightOpenRange[0] === 'number') {
            expressions.push(
              ident + '>=' + filters[i].$rightOpenRange[0]
              + ' AND ' + ident + '<' + filters[i].$rightOpenRange[1]
            );
          } else {
            throw new Error(ident + ' of \'' + typeof filters[i].$rightOpenRange[0] + '\' can not be used in ranged queries.');
          }
        } else {
          throw new Error('Range bounds types don\'t match');
        }
      }
